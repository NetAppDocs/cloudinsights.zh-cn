...
* 必须提供以下命令：_curl、sudo、openssl、sha256sum、_和_kubectl_。为获得最佳结果，请将这些命令添加到路径中。请注意、至少需要为_kubectl_配置对以下Kubernetes对象的访问权限：_agents、clusterroles、clusterrolebindings、customresourcesdefiniments、deployments、 命名空间、角色、轮绑定、机密、服务帐户、_和_services_。请参见 link:nkmo_clusterrole_yaml.html["此处"] 具有这些最低clusterrole特权的.yaml文件示例。
...
....
根据您的企业策略，将操作员 Docker 映像推送到您的私有 / 本地 / 企业 Docker 存储库。确保存储库中这些映像的目录路径与Docker <cluster>.cloudinsights.netapp.com中的路径一致。
