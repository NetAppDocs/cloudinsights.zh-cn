...
[role="lead"]
Cloud Insights 提供了四个用户帐户：帐户所有者，管理员，用户和来宾。系统会为每个帐户分配特定的权限级别。用户可以是其中之一 link:#creating-accounts-by-inviting-users["已邀请"] Cloud Insights 并分配了特定角色，或者可以通过登录  具有默认角色。SSO 作为 Cloud Insights 高级版的一项功能提供。



== 权限级别

您可以使用具有管理员权限的帐户来创建或修改用户帐户。系统会为每个用户帐户分配以下权限级别之一。

* * 来宾 * 可以查看资产页面，信息板和查询，并运行查询。
* * 用户 * 可以执行所有子系统级别的特权，也可以创建，修改或删除信息板，查询，标注， 标注规则和应用程序。
* * 管理员 * 和 * 帐户所有者 * 可以执行所有功能，还可以创建，修改和删除策略，导入信息板以及管理所有用户和数据收集器。


帐户所有者是在注册 Cloud Insights 时创建的。

最佳实践是限制具有管理员权限的用户数量。最大数量的帐户应为用户或来宾帐户。


NOTE: 强烈建议每个 Cloud Insights 环境至少有两个帐户所有者。 
...
要配置 _SSO 用户自动配置 _ ，请在 * 管理员 > 用户管理 * 页面上单击 * 配置 SSO* 按钮。配置后，管理员可以启用 SSO 用户登录。当管理员启用 _SSO 用户自动配置 _ 时，他们会为所有 SSO 用户（如来宾或用户）选择一个默认角色。通过 SSO 登录的用户将具有此默认角色。

image:SSOBanner.jpg["使用 SSO 进行用户管理"]
...
image:UserListWithSSO.png["已启用 SSO"]
