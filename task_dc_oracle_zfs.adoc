...
| " 配置错误 " ，并显示错误消息 " 用户未授权命令 " | 此错误可能是由于配置的用户未包含某些角色(例如、"advanced_Analytics ")。对具有只读角色的用户应用分析范围可能会删除此错误。请按照以下步骤操作：1.在ZFS系统上，从“Configuration -> Users”(配置->用户)屏幕中，将鼠标移动到角色上并双击以允许编辑2.从范围下拉菜单中选择"分析"。此时将显示可能属性的列表。3.单击最顶部的复选框、它将选择所有三个属性。4.单击右侧的添加按钮。5.单击弹出窗口右上角的应用按钮。此时将关闭弹出窗口。 
|===
