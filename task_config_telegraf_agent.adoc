...
NOTE: 基于操作员的安装被视为 _Preview_ 功能，因此可能会发生更改。
...
* 基于操作员的安装已通过测试，预计可与 AWS EKS 1.18 和 OpenShift 3.11 配合使用。
...
....
如果先前为 Telegraf 创建了安全上下文限制（指向 " 配置代理以从 Kubernetes 收集数据 " 的超链接）：
...
| 在 Kubernetes 上，我的 Telegraf Pod 报告以下错误： " 处理 mountstats 信息时出错：无法打开 mountstats 文件： /hostfs/proc/1/mountstats ，错误： open /hostfs/proc/1/mountstats ：权限被拒绝 " | 如果启用并强制实施 SELinux ，则可能会阻止 Telegraf Pod 访问 Kubernetes 节点上的 /proc/1/mountstats 文件。要放宽此限制，请执行以下操作之一： •在运行 Telegraf 的每个 Kubernetes 节点上，运行 `chcon -Rt container_runtime_t /proc/1/mountstats` •在运行 Telegraf 的每个 Kubernetes 节点上，将 SELinux 安全策略从 " 强制实施 " 更改为 " 宽松 " •编辑 Telegraf Demonf 并将 spec.containers.securityContext.privileged 设置为 "true" 
|===
