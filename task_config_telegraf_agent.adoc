...
| 配置新插件并重新启动Telegraf后、Telegraf无法启动。这些日志指示出现类似以下内容的错误："[caelaf] Error running agent：error loading config file /etc/caelaf/tengraf.d/cloudinsots-default.conf：plugin outputs.http：line 88：configuration specified the fields ["use_system_proxy"]、but they were unused" | 安装的Telegraf版本已过时。按照此页面上的步骤*为您的相应平台升级代理*。 
