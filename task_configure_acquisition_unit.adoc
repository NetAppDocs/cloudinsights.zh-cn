...
阅读了解Cloud Insights安全性 link:security_overview.html["此处"]。
...
. 单击*AObservability > Collectors > Acquisition Unit >+Acquisition Unit*
+
...
. 在Cloud Insights中，转至*Observability > Collectors并选择*Acquisition Unit*选项卡。
...
. 登录到Cloud Insights环境并转到*Observability > Collectors *。
