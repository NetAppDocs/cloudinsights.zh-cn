...
summary: 正在配置 task _dc_fitsu_ 数据收集器。 
---
= 3PAR StoreServ 存储数据收集器


[role="lead"]
Cloud Insights 使用 3PAR StoreServ 数据收集器来发现 HP 3PAR StoreServ 存储阵列的清单和性能。



== 术语

Cloud Insights 从 3PAR StoreServ 数据收集器采集以下清单信息。对于 Cloud Insights 收购的每种资产类型，均显示了此资产最常用的术语。查看此数据收集器或对其进行故障排除时，请记住以下术语：
...
* 端口要求： 22 （清单收集）， 5988 或 5989 （性能收集） [ 注意： StoreServ OS 3.x+ 支持 3PAR 性能 ]
...
| "showsys" 命令不返回任何结果。 | 从命令行运行 "showsys" 和 "showversion -a" ，并检查 3PAR 是否支持此版本。 
|===
