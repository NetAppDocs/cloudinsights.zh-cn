...
* 编辑 Telegraf ConfigMap （ CM ），并将旧 API 令牌的所有实例替换为新 API 令牌。
+
....
...
:wq!
....
* 编辑 Telegraf DemonSet （ DS ）和 ReplicaSet （ RS ），并将旧 API 令牌的所有实例替换为新 API 令牌。
+
....
...
:wq!
....
* Kubernetes 控制器将自动重新启动 Telegraf DS Pod 。
* 重新启动 Telegraf RS Pod
+
