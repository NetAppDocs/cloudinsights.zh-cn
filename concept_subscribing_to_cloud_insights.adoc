...
NOTE: 除非另有说明、否则本页面上的信息通常为 适用场景 Cloud Insights 商用版(基础版、标准版、高级版)。Cloud Insights 联邦版可能不包含本页所述的某些功能。
...
* *联邦版*提供符合FedRAMP标准的美国Cloud Insights 版政府部门。上述商业版中提供的某些功能可能在联邦版中不提供、或者可能随功能减少而提供。在可能的情况下、Cloud Insights 文档会记录这些差异。
...
| 实时聊天^&plus；&plus；^和技术支持 | - | 检查（ &C ）； | 检查（ &C ）； 
...
| 业务智能和报告(&ast)； | - | - | 检查（ &C ）； 
|===
上次；一次只能有5个活动的自定义监视器[&A];API访问因版本而异(&A);上次;可用于包含500个受管单元和更大的环境^&PLUS；^功能在Cloud Insights 联邦版中不可用
