...
. 添加REST收集器、输入所有相同的用户/权限等 您已为上一个收集器设置了。
. 暂停上一个收集器。
. 让系统收集~30分钟的数据。忽略在此期间的任何数据古怪。
. 休息期结束后、REST收集器上的数据将稳定持续。
...
* 帐户权限：
+
** 在功能上、Cloud Insights会发出读取请求、但要向ONTAP阵列注册Cloud Insights、需要一些写入权限。请参见 <<a-note-about-permissions,有关权限的注意事项>> 。
