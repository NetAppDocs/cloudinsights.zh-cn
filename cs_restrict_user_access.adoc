...
NOTE: 使用Amazon FSX数据收集器时、SMB的用户阻止无法终止当前用户会话。请参见 ,故障排除 部分、了解更多信息。
...
| 我看到错误：svm1：用户域\\user01的现有CIFS会话未关闭。此错误会显示在警报详细信息页面上的"Action take"部分以及"Alert & User List"页面下的"Access Limtion History"下。如果出现此错误、则不会关闭用户当前会话、但是、在存储期限到期之前、系统会阻止用户进行任何新会话。 | 这是Amazon FSX中的已知问题描述。Cloud Secure 将无法关闭现有SMB会话。当前没有可用于阻止Amazon FSX的现有SMB会话的临时解决策。如果收集器类型为CVO或ONTAP 、请确保中所述的权限正确 ,前提条件 部分。 
|===