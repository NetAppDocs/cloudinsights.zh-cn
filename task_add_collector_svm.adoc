...
| 启用 Cloud Secure 后， ONTAP 性能将受到影响：延迟偶尔会高， IOPS 偶尔会低。 | 确保您使用的是 Data ONTAP 版本，其中 link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1415152["此问题描述"] 已修复。建议的 ONTAP 最低版本为 9.8P7 。如果使用的是 9.8p7 之前的 ONTAP 版本，并且显示了此性能问题描述，则可以更改 ONTAP 发送缓冲区大小以提高 ONTAP 性能。如果您未看到此选项，但希望了解此选项，请联系 NetApp 支持部门。 
