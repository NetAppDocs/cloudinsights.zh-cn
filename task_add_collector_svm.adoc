...
** SMB协议3.1及更早版本。  请注意、工作负载安全性不适用于使用FlexCache 的SMB配置。从ONTAP 9.7开始、在使用FlexCache的系统中、Fpolicy仅在NFS环境中受支持。
** NFS 协议 4.0 及更早版本
...
如果Data Collector处于_running"或"_Error"状态、则可以暂停收集。打开收集器的"三点"菜单、然后选择暂停。暂停收集器时、不会从ONTAP收集任何数据、也不会从收集器向ONTAP发送任何数据。这意味着不会有Fpolicy事件从ONTAP流向数据收集器、也不会从该数据收集器流向Cloud Insights。
...
| 1. | 根据已暂停收集器上配置的设置、不会执行Snapshot清除。 


| 2. | 暂停的收集器不会处理EMS事件(如ONTAP ARP)。这意味着、如果ONTAP发现勒索软件攻击、Cloud Insights工作负载安全性将无法获取该事件。 


| 3. | 不会为已暂停的收集器发送运行状况通知电子邮件。 


| 4. | 暂停的收集器不支持手动或自动操作(例如Snapshot或用户阻止)。 


| 5. | 在代理或收集器升级、代理VM重新启动/重新启动或代理服务重新启动时、暂停的收集器将保持_Paused. 


| 6. | 如果数据收集器处于_Error_状态、则无法将此收集器更改为_Paused _状态。只有当收集器的状态为_running"时、暂停按钮才会启用。 


| 7. | 如果代理已断开连接、则无法将收集器更改为_Paused _状态。收集器将进入_STOPPED _状态、暂停按钮将被禁用。 
|===
...
| 启用工作负载安全性后、ONTAP 性能将受到影响：延迟偶尔会高、IOPS偶尔会低。 | 确保您使用的是 Data ONTAP 版本，其中 link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1415152["此问题描述"] 已修复。建议的 ONTAP 最低版本为 9.8P7 。如果使用的是 9.8p7 之前的 ONTAP 版本，并且显示了此性能问题描述，则可以更改 ONTAP 发送缓冲区大小以提高 ONTAP 性能。如果您希望浏览此选项，但在添加新数据收集器或编辑现有数据收集器时看不到此设置，请联系 NetApp 支持部门。 
