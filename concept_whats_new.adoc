...
。 link:/task_config_telegraf_agent_k8s.html#operator-based-install-or-script-based-install["NetApp Kubernetes监控操作员"] (NKMO）适用于运行Kubernetes 1.17或更高版本的所有环境、是安装适用于Cloud Insights 的Kubernetes的首选方法。与脚本安装的监控方法相比、NKMOO是一项改进、因为它可以更灵活地配置可通过Cloud Insights 进行控制的监控、并减少客户驱动的配置干预、同时还可以增加监控K8s集群中运行的其他软件的机会。
...
现在、您可以接收数据收集警报 link:/task_system_monitors.html#collector-monitors["事件"]：
...
请注意、这些监控器默认为_Paused__、必须先启用(恢复)、然后才能发送警报。

另请注意、收集器警告的监控器可以发送比其他监控器类型更多的警报、因此建议不要设置任何警报收件人、除非您正在进行故障排除。



==== 有关存储更改日志事件的警报

在为Cloud Insights 生成的存储更改日志事件定义监控器(使用日志源_logs.cloud_insights.change_log.storage_)时、会收到信息警报、并在每次收集后比较存储系统的过去和新状态。对于以下事件、ONTAP 存储可以使用默认监控器：
...
| Cloud Secure —在受到攻击时限制用户访问 | 可以在检测到攻击时限制用户访问，从而增强对业务关键型数据的保护。可以使用自动响应策略自动限制访问，也可以从警报或用户详细信息页面手动限制访问。link:https://docs.netapp.com/us-en/cloudinsights/cs_automated_response_policies.html["阅读更多内容"] 
...
* * Cohesity Smart Files* —此基于 REST API 的收集器将获取 Cohesity 集群，发现 " 视图 " （作为 CI 内部卷），各个节点以及收集性能指标。
